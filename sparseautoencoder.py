...
def sampleIMAGES():
    ...
    IMAGES = np.load('C:/Users/georg/.spyder-py3/Homework 3/images.npy')  # load images from disk
    ...
    for i in range(0,numpatches):
        r1 = np.random.randint(0,512-patchsize)
        r2 = np.random.randint(0,512-patchsize)
        r3 = np.random.randint(0,10)
        
        img = IMAGES[r1:r1+8,r2:r2+8,r3]
        training_point = img.reshape(1,patchsize*patchsize)
        patches[i,:] = training_point
...
def sparseAutoencoderCost(theta,visibleSize,hiddenSize,decayWeight,sparsityParam,beta,data):
    ...
    # get the size of the training data set
    samples_num,datalength = data.shape
    error_cost = 0
    
    
    
    #initialize a, h, z, W1 partial, W2 partial, b1 partial, b2 partial
    z_2 = np.zeros([hiddenSize,samples_num])
    z_3 = np.zeros([visibleSize,samples_num])
    a_2 = np.zeros([hiddenSize,samples_num])
    h_out = np.zeros([visibleSize,samples_num])
    p = np.zeros([hiddenSize,1])
    W1partial = np.zeros(np.shape(W1))
    W2partial = np.zeros(np.shape(W2))
    b1partial = np.zeros(np.shape(b1))
    b2partial = np.zeros(np.shape(b2))
    
    
    # iterate through each training sample, storing z, a, h
    for iSample in range(0,samples_num):
        
        #first feed the data forward and calculate a and h
        training_point = data[iSample,:]
        z_2[:,iSample] = np.array(np.dot(W1.T,training_point)+b1)
        
        # for each hidden node, calculate the activation
        for iHiddenNode in range(0,hiddenSize):
            a_2[iHiddenNode,iSample] = sigmoid(z_2[iHiddenNode,iSample])
            
        z_3[:,iSample] = np.array(np.dot(W2.T,a_2[:,iSample])+b2)
        
        # for each output node, calculate the activation
        for iOutputNode in range(0,visibleSize):
            h_out[iOutputNode,iSample] = sigmoid(z_3[iOutputNode,iSample])
    
    #calculate p for each hidden node  which is the output from the hidden layer and overall output
    for iHiddenNode in range(0,hiddenSize):
         p[iHiddenNode] = (1/samples_num)*sum(a_2[iHiddenNode,:])
        
    # for each sample, calculate f, d partial derivatives and error terms
    for iSample in range(0,samples_num):
        training_point = data[iSample,:]
        
        # calculate f in terms of an identity matrix
        f_out = (h_out[:,iSample]*np.identity(visibleSize))*((1-h_out[:,iSample])*np.identity(visibleSize))
        f_2 = (a_2[:,iSample]*np.identity(hiddenSize))*((1-a_2[:,iSample])*np.identity(hiddenSize))
        
        #calculate d_out and d_2, including sparsity term in d_2
        d_out = np.dot(f_out,-(training_point-h_out[:,iSample]))
        d_2 = np.dot(f_2,np.array([np.dot(W2,d_out)]).T+(beta*((-sparsityParam/p)+(1-sparsityParam)/(1-p))))
        
        #calculate partial derivatives for the current sample
        W1partial = W1partial+np.dot(d_2,np.array([training_point])).T
        W2partial = W2partial+np.dot(np.array([d_out]).T,np.array([a_2[:,iSample]])).T
        b1partial = b1partial+d_2[:,0]
        b2partial = b2partial+d_out
        
        # compute the squared error cost for this sample
        error_cost = error_cost + .5*(np.linalg.norm(h_out[:,iSample]-training_point)*np.linalg.norm(h_out[:,iSample]-training_point))
        
    #Compute final gradients
    W1grad = (W1partial/samples_num)+decayWeight*W1
    W2grad = (W2partial/samples_num)+decayWeight*W2
    b1grad = b1partial/samples_num
    b2grad = b2partial/samples_num
    
    # Compute decay cost term
    W1_decay_term = sum(sum(np.square(W1)))
    W2_decay_term = sum(sum(np.square(W2)))
    decay_term_cost = (decayWeight/2)*(W1_decay_term+W2_decay_term)
    
    #compute sparsity cost term
    sparsity_cost_term = beta*sum(sparsityParam*np.log(sparsityParam/p)+(1-sparsityParam)*np.log((1-sparsityParam)/(1-p)))
    
    cost = (error_cost/samples_num)+decay_term_cost+sparsity_cost_term
...
def computeNumericalGradient(J,theta):
    ...
    theta_length = np.shape(theta)
    e = np.zeros(theta_length[0])
    
    for itheta in range(0,theta_length[0]):
        if itheta == 0:
            e[itheta] = 1
        else:
            e[itheta] = 1
        ...
        numgrad[itheta] = (J(theta+eps*e)-J(theta-eps*e))/(2*eps)
    ## ---------------------------------------------------------------
    return numgrad
...
def displayNetwork(A,optNormEach=False,optNormAll=True,numColumns=None,imageWidth=None,cmapName='gray',
    ...
    nr,nc = np.shape(A) 
    if imageWidth==None:
        sx = np.long(np.ceil(np.sqrt(nc)))
        sy = np.long(np.ceil(nc/sx))
    else:
        sx = np.long(imageWidth)
        sy = np.long(np.ceil(nc/sx))
    if numColumns==None:
